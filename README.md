...
**ジャンル（武将・合戦・城）× 難易度（初級・中級・上級）の9コース**構成で、各コース **20問・合計180問**。
正誤判定・解説・スコア・正答率ベースのランク判定まで実装しています。
バックエンド／DB は不要で、クイズデータはコード内に持っています。

ユーザーはトップ画面で「ジャンル → 難易度」を選び、その20問だけを解きます。
...
この分離により、`QuizData.fs` の `getQuestions category difficulty` の中身を差し替えるだけで、
将来 **Supabase / Firebase / 外部API** からデータを取る構成に移行できます（UI・状態側は変更不要）。

### ジャンル × 難易度の管理

- **内部キーと表示名を分離**しています（`Types.fs`）。
  - ジャンル：`Busho`/`Battle`/`Castle` ↔ 「武将」「合戦」「城」（`Category.displayName`）
  - 難易度：`Beginner`/`Intermediate`/`Advanced` ↔ 「初級」「中級」「上級」（`Difficulty.displayName`）
- 1コース分の20問は `QuizData.getQuestions category difficulty` で取得します
  （内部では各ジャンルの配列を `Difficulty` で絞り込み。論理的には `quizData[category][difficulty]` に相当）。

---

## クイズデータをどこで編集できるか

すべて **`src/QuizData.fs`** にあります。ジャンルごとに 3 つのリスト（`busho` / `battle` / `castle`）があり、
各リストの中に初級20・中級20・上級20問（`Difficulty` で区別）が入っています。
...
1. `src/QuizData.fs` の対象リスト（例：`busho`）に、追加したい難易度の
   `{ Id = ...; ...; Difficulty = "上級" }` を**追記**するだけ。
   `Id` は同じジャンル・同じ難易度内で重複しない値にしてください。
2. 出題数は「そのコース（ジャンル×難易度）の全件」を使う設計なので、**追加すれば自動で出題数が増えます**
   （進捗バー・「◯ / 全問数」表示・結果画面の分母もすべて自動追従します）。
3. もし「1コースを必ず20問だけにしたい」場合は、`State.fs` の `startQuiz` 内で
   `questions |> List.truncate 20` のように件数を絞れます。
4. 追加後は、起動時のコンソールに出る `validateQuizData` の検証結果で、
   各コースの問題数・重複・answer整合を確認できます。
...
- トップの**丸いジャンルカード**（色分け・順番に出現）＋直下の**難易度ボタン（初級/中級/上級）**
  で「ジャンル×難易度」の9コースから選択
- クイズ画面：**ジャンル名｜難易度**／問題番号／全問数／スコア／問題文／4択（A〜D）／戻る／次へ
- **正誤表示**：正解＝緑○、不正解＝選択肢が赤×＋正解を緑で提示、回答後は他を押せない
- **解説文**と「次の問題へ／結果を見る」ボタン
- 結果画面：**ジャンル名｜難易度**／正解数／**正答率(%)**／**ランク**／コメント／「同じジャンル・難易度でもう一度」「トップに戻る」
...
問題数が何問でも同じ基準で判定できるよう、**正答率（%）** でランクを決めています（`State.fs` の `calculateRank`）。
