...
各ジャンル **50問ずつ・合計150問**、正誤判定・解説・スコア・ランク判定まで実装しています。
バックエンド／DB は不要で、クイズデータはコード内に持っています。
各問題には難易度（初級20／中級20／上級10）を示す `Difficulty` を付与しています（将来の難易度選択機能用）。
...
（各カテゴリの問題数・選択肢が4つか・answer が options に含まれるか・id/問題文の重複がないか）。
